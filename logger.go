// Copyright 2017 github.com/snippetor/logger Author. All Rights Reserved.
//
...
func (l *logger) rollingFile(date string) {
	...
	if l.config.LogFileRollingType&RollingSize == RollingSize {
		...
		for l.isFileExists(filepath.Join(l.config.LogFileOutputDir, tmpFileName)) {
			tmpFileName = newFileName + "." + strconv.Itoa(i)
			i += 1
		}
...
func (l *logger) isFileExists(name string) bool {
